  ...
  try {
    ...
    switch (event.type) {
      ...
      case 'checkout.session.completed':
        ...
        if (event.data.object.payment_status === "paid") {  
          if (checkout_session_completed.customer_details?.email && checkout_session_completed?.metadata) {
            let idRequestSend = checkout_session_completed.metadata.idUser
      ...
      case 'checkout.session.expired':
        ...
        const res = await prisma.requestSend.findMany({
        ...
        await deleteFolderReq(res[0]?.idRequestSend)
        await deleteReqById(res[0]?.idRequestSend)
